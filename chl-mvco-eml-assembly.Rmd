...
chl_input <- read_excel(here('MVCO_chl_4EDI_copy20260210.xlsx')) 
...
df <- read_excel('List of MVCO cruises for NES-LTER project 20260210.xlsx', sheet = "catalog", skip = 6)
...
```{r}
chl_url <- glue::glue('{base_api_url}/chl/all')

chl_transect <- tryCatch(read_csv(chl_url), 
                 error=function(cond) 
                     { message(cond)
                       return(NULL) })
...
filtered_chl_aligned <- filtered_chl_aligned %>%
  dplyr::mutate(date_time_utc = lubridate::ymd_hms(date_time_utc, tz = "UTC"))
...
package_id <- "knb-lter-nes.21.1"
entity_id  <- "2a7f068e3256f6453e1b31fc247f5c6f"

csv_lines <- read_data_entity(packageId = package_id, entityId = entity_id)
chl_mvco_pubd <- read_csv(I(csv_lines), show_col_types = FALSE)

chl_mvco_pubd <- chl_mvco_pubd %>%
  rename(cruise = cruise_ID)

chl_mvco_pubd <- chl_mvco_pubd[, names(chl_mvco)]
